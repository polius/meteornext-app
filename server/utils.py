...
class Utils:
    ...
    def prepare(self, data, force=False):
        if not data['cross_region']:
            return
        ...
        if not self.check_path(data['deploy_path']):
            return {'success': False, 'error': "The local logs path has no write permissions"}

        # Check if Meteor is already deployed
        result = self.__ssh("test -d '{}' && printf '1' || printf '0'".format(data['deploy_path']))

        if result['stdout'][0] != '1' or force:
            # Deploy Meteor to the SSH Region
            self.__ssh("mkdir -p {0} && chmod 700 {0} && rm -rf {0}/*".format(data['deploy_path']))
            if not is_binary:
            ...
            self.__put("{}/meteor.tar.gz".format(script_path), "{}/meteor.tar.gz".format(data['deploy_path']))
            self.__ssh("cd {} && tar -xvzf meteor.tar.gz -C . && rm -rf meteor.tar.gz".format(data['deploy_path']))
    ...
    def unprepare(self, data):
        ...
        self.__ssh("rm -rf {}".format(data['deploy_path']))
        ...
        for i in range(attempts):
            try:
                if self._conn:
                    ...
                    with sshtunnel.SSHTunnelForwarder((self._conn['hostname'], int(self._conn['port'])), ssh_username=self._conn['username'], ssh_password=self._conn['password'], ssh_pkey=self._conn['key'], remote_bind_address=(server['hostname'], int(server['port']))) as tunnel:
