...
class Restore:
    ...
    def post(self, user, data):
        query = """
            INSERT INTO restore (`mode`, `details`, `source`, `selected`, `size`, `server_id`, `database`, `uri`, `upload`, `user_id`)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """
        return self._sql.execute(query, (data['mode'], data['details'], data['source'], data['selected'], data['size'], data['server_id'], data['database'], data['uri'], data['upload'], user['id']))
