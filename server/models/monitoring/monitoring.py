class Monitoring:
    ...
    def put_monitor(self, group_id, data):
        if len(data) == 0:
            query = """
                UPDATE monitoring
                JOIN servers s ON s.id = monitoring.server_id
    ...
    def put_processlist(self, group_id, data):
        if len(data) == 0:
            query = """
                UPDATE monitoring
                JOIN servers s ON s.id = monitoring.server_id
            ...
            for i in data:
                query = """
                    INSERT INTO monitoring (server_id, processlist_enabled, processlist_active)
                    SELECT s.id, 1, 1
                    FROM servers s
                    JOIN regions r ON r.id = s.region_id
                    WHERE s.id = %s
    ...
    def deactivate_processlist(self, group_id):
        query = """
            UPDATE monitoring
            JOIN servers s ON s.id = monitoring.server_id
            JOIN regions r ON r.id = s.region_id AND r.group_id = %s
