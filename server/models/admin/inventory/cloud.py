...
class Cloud:
    ...
    def get(self, group_id=None, cloud_id=None, user_id=None):
        if user_id is not None:
            query = """
                SELECT c.id, c.name, c.group_id, g.name AS 'group', c.type, c.access_key, c.secret_key, c.shared, c.owner_id, u.username AS 'owner', u2.username AS 'created_by', c.created_at, u3.username AS 'updated_by', c.updated_at
                FROM cloud c
    ...
    def post(self, user, cloud):
        ...
        """
        self._sql.execute(query, (cloud['name'], cloud['group_id'], cloud['type'], cloud['access_key'], cloud['secret_key'], cloud['shared'], cloud['shared'], cloud['owner_id'], user['id'], datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")))
