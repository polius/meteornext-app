    ...
    def get_queries(self, dfilter=None, dsort=None):
        ...
        if dfilter is not None:
            ...
            if 'server' in dfilter and dfilter['server'] is not None:
                server = 'AND s.name = %(server)s'
                args['server'] = dfilter['server']
            if 'database' in dfilter and len(dfilter['database']) > 0 and 'databaseFilter' in dfilter and dfilter['databaseFilter'] in matching:
                database = f"AND cq.database {matching[dfilter['databaseFilter']]['operator']} %(database)s"
                args['database'] = matching[dfilter['databaseFilter']]['args'].format(dfilter['database'])
            if 'query' in dfilter and len(dfilter['query']) > 0 and 'queryFilter' in dfilter and dfilter['queryFilter'] in matching:
                query = f"AND cq.query {matching[dfilter['queryFilter']]['operator']} %(query)s"
                args['query'] = matching[dfilter['queryFilter']]['args'].format(dfilter['query'])
            if 'status' in dfilter and dfilter['status'] is not None and len(dfilter['status']) > 0:
                status = 'AND cq.status IN (%s)' % ','.join([f"%(status{i})s" for i in range(len(dfilter['status']))])
                for i,v in enumerate(dfilter['status']):
                    args[f'status{i}'] = v
            if 'startDateFrom' in dfilter and len(dfilter['startDateFrom']) > 0:
                start_date_from = 'AND cq.start_date >= %(start_date_from)s'
                args['start_date_from'] = dfilter['startDateFrom']
            if 'startDateTo' in dfilter and len(dfilter['startDateTo']) > 0:
                start_date_to = 'AND cq.start_date <= %(start_date_to)s'
                args['start_date_to'] = dfilter['startDateTo']
            if 'endDateFrom' in dfilter and len(dfilter['endDateFrom']) > 0:
                start_date_from = 'AND cq.end_date >= %(end_date_from)s'
                args['end_date_from'] = dfilter['endDateFrom']
            if 'endDateTo' in dfilter and len(dfilter['endDateTo']) > 0:
                end_date_to = 'AND cq.end_date <= %(end_date_to)s'
                args['end_date_to'] = dfilter['endDateTo']
