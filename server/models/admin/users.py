...
class Users:
        ...
        if username is None:
            ...
            query = """
                SELECT
                    ...
                    g.coins_day, g.inventory_enabled, g.deployments_enabled, g.deployments_basic, g.deployments_pro, g.deployments_coins, g.monitoring_enabled, g.utils_enabled, g.utils_coins, g.client_enabled,
                    CASE
                        WHEN mfa.2fa_hash IS NOT NULL THEN '2fa'
                        WHEN mfa.webauthn_pub_key IS NOT NULL THEN 'webauthn'
                        ELSE NULL
    ...
    def delete(self, users):
        for user in users:
            ...
            self._sql.execute("DELETE es FROM executions_scheduled es JOIN executions e ON e.id = es.execution_id JOIN deployments d ON d.id = e.deployment_id JOIN users u ON u.id = d.user_id AND u.username = %s", (user))
