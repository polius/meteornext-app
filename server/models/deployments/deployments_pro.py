...
class Deployments_Pro:
    ...
    def get(self, user_id, execution_id):
        query = """
            SELECT d.id, p.id AS 'execution_id', d.mode, d.name, e.name AS 'environment', p.code, p.method, p.status, p.created, p.started, p.ended, CONCAT(TIMEDIFF(p.ended, p.started)) AS 'overall', p.error, p.progress, p.uri, p.engine
