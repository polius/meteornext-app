...
class Deployments_Basic:
    ...
    def updateStatus(self, deployment_id, status, extra=None):
        if extra is None:
            ...
            query = "UPDATE deployments_basic SET `status` = 'STARTING' WHERE id = %s AND status != 'STOPPED'"
            self._sql.execute(query, (deployment_id))
        elif status == 'STOPPING':
            query = "UPDATE deployments_basic SET `status` = 'STOPPING', `stopped` = %s WHERE id = %s AND `status` IN('STARTING','IN PROGRESS')"
            self._sql.execute(query, (extra, deployment_id))
        elif status == 'STOPPED':
