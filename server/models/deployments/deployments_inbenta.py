...
class Deployments_Inbenta:
    ...
    def get(self, execution_id):
        query = """
            SELECT d.id, i.id AS 'execution_id', 'INBENTA' AS 'mode', d.name, r.name AS 'release', e.name AS 'environment', i.products, i.schema, i.databases, i.queries, i.method, i.status, q.queue, i.created, i.scheduled, i.started, i.ended, CONCAT(TIMEDIFF(i.ended, i.started)) AS 'overall', i.error, i.progress, i.url, i.uri, i.engine, i.public
    ...
    def post(self, deployment):
        ...
        query = """
            INSERT INTO deployments_inbenta (deployment_id, environment_id, `products`, `schema`, `databases`, queries, method, `status`, created, scheduled, url)
            SELECT %s, e.id, %s, %s, %s, %s, %s, %s, %s, IF(%s = '', NULL, %s), %s
            FROM environments e
            WHERE e.name = %s
            AND e.group_id = %s
        """
        return self._sql.execute(query, (deployment['id'], products, deployment['schema'], deployment['databases'], str(deployment['queries']), deployment['method'], deployment['status'], datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"), deployment['scheduled'], deployment['scheduled'], deployment['url'], deployment['environment'], deployment['group_id']))
    ...
    def getExecutionsN(self, execution_ids):
        query = """
            SELECT i.id AS 'execution_id', 'INBENTA' AS 'mode', u.username AS 'user', g.id AS 'group_id', e.name AS 'environment', i.products, i.schema, i.databases, i.queries, i.method, g.deployments_execution_threads AS 'execution_threads', g.deployments_execution_limit AS 'execution_limit', i.url
            FROM deployments_inbenta i
            JOIN deployments d ON d.id = i.deployment_id
            JOIN environments e ON e.id = i.environment_id
            JOIN users u ON u.id = d.user_id
