...
class Servers:
    ...
    def get(self, user):
        ...
        if user['inventory_secured'] and not user['owner']:
            ...
            for s in servers:
                if s['shared']:
                    servers_secured.append({"id": s['id'], "name": s['name'], "region": s['region'], "engine": s['engine'], "version": s['version'], "shared": s['shared'], "region_shared": s['region_shared'], "usage": s['usage']})
                else:
