...
class query:
    def __init__(self, args, imports, region=None):
        self._args = args
    ...
    def start_sql_connection(self, server, tunnel):
        self._sql_credentials = server
        self._sql_connection = mysql(self._args, {"sql": server, "tunnel": tunnel})
        ...
        if self._args.deploy or query_syntax == 'Select':
            try:
                ...
                if self._args.execution_limit and query_syntax == 'Select':
                    ...
                    if epf > int(self._args.execution_limit):
