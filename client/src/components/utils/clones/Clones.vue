  ...
  <div>
    <v-card>
      <v-toolbar dense flat color="primary">
        <v-toolbar-title class="white--text subtitle-1">CLONES</v-toolbar-title>
        <v-divider class="mx-3" inset vertical></v-divider>
        <v-toolbar-items>
          <v-btn text @click="newClone()"><v-icon small style="margin-right:10px;">fas fa-plus</v-icon>NEW</v-btn>
          <v-btn :disabled="selected.length != 1" text @click="infoClone()"><v-icon small style="padding-right:10px">fas fa-bookmark</v-icon>DETAILS</v-btn>
          <v-btn :disabled="selected.length == 0" text @click="deleteClone()"><v-icon small style="margin-right:10px;">fas fa-minus</v-icon>DELETE</v-btn>
          <v-divider class="mx-3" inset vertical></v-divider>
          <v-btn @click="getClones" text><v-icon small style="margin-right:10px">fas fa-sync-alt</v-icon>REFRESH</v-btn>
        </v-toolbar-items>
        <v-divider class="mx-3" inset vertical></v-divider>
      ...
      <v-data-table v-model="selected" :headers="computedHeaders" :items="items" @current-items="(items) => current = items" :loading="loading" loading-text="Loading... Please wait" item-key="id" show-select class="elevation-1" style="padding-top:5px;" mobile-breakpoint="0">
      ...
      <v-card>
        ...
        <v-card-text style="padding:15px">
          <v-container style="padding:0px">
            <v-layout wrap>
              <v-flex xs12>
                <div class="subtitle-1" style="margin-bottom:10px">Are you sure you want to delete the selected clones?</div>
                <v-divider></v-divider>
...
export default {
  ...
  methods: {
    ...
    deleteSubmit() {
      ...
      axios.delete('/utils/clones', { data: payload })
        .then(() => {
          this.selected = []
          this.deleteDialog = false
          this.getClones()
    ...
    newClone() {
      this.$router.push({ name: 'utils.clones.new' })
    },
    infoClone() {
