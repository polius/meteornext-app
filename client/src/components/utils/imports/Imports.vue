...
export default {
  data: () => ({
    ...
    columnsDialog: false,
    columns: ['mode','server_id','database','status','created','started','ended','overall'],
    columnsRaw: [],
  }),
  created() {
    this.getImports()
  ...
  methods: {
    getImports() {
      ...
      axios.get('/utils/imports')
        .then((response) => {
          this.origin = response.data.imports.map(x => ({...x, source: x.mode == 'cloud' ? JSON.parse(x.details)['bucket'] + '/' + x.source : x.source, created: this.dateFormat(x.created), started: this.dateFormat(x.started), ended: this.dateFormat(x.ended)}))
    ...
    selectAllColumns() {
      this.columnsRaw = ['mode','source','size','server_id','database','status','created','started','ended','overall']
    },
    deselectAllColumns() {
      this.columnsRaw = []
    },
