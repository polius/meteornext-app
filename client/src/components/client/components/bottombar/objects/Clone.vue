              ...
              <v-flex xs12>
                <v-card>
                  <v-row no-gutters align="center" justify="center">
                    ...
                    <v-col>
                      <div class="text-body-1" style="color:#e2e2e2">To clone objects larger than 10 MB use the Utils section.</div>
                    </v-col>
                  </v-row>
                </v-card>
    ...
    cloneObjectsSubmit() {
      ...
      for (let i of this.gridApi['tables'].getSelectedRows()) {
        if (i['data_length']/1024/1024 > 10) {
          EventBus.$emit('send-notification', 'To clone objects larger than 10 MB use the Utils section.', '#EF5354')
          return
        }
      }
      // Init Clone
