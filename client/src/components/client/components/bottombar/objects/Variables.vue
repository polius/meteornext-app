      ...
      <v-card>
        ...
        <v-card-text style="padding:0px;">
          <v-container style="padding:0px; max-width:100%;">
            <v-layout wrap>
              <v-flex xs12>
                <ag-grid-vue suppressDragLeaveHidesColumns suppressColumnVirtualisation suppressRowClickSelection oncontextmenu="return false" @grid-ready="onGridReady" @first-data-rendered="onFirstDataRendered" @cell-key-down="onCellKeyDown" @cell-editing-started="cellEditingStarted" @cell-editing-stopped="cellEditingStopped" :stopEditingWhenCellsLoseFocus="true" style="width:100%; height:70vh;" class="ag-theme-alpine-dark" rowHeight="35" headerHeight="35" rowSelection="single" :columnDefs="columns" :rowData="items"></ag-grid-vue>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card-text>
    ...
    <v-dialog v-model="confirmDialog" max-width="50%">
      <v-card>
        <v-toolbar dense flat color="primary">
          <v-toolbar-title class="white--text subtitle-1"><v-icon small style="margin-right:10px; margin-bottom:3px">fas fa-exclamation-triangle</v-icon>CONFIRMATION</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-btn :disabled="loading" @click="confirmDialogCancel" icon><v-icon size="22">fas fa-times-circle</v-icon></v-btn>
        </v-toolbar>
        <v-card-text style="padding:15px">
          <v-container style="padding:0px; max-width:100%;">
            <v-layout wrap>
              <v-flex xs12>
                <div class="body-1">Are you sure you want to change this server variable?</div>
  ...
  methods: {
    ...
    confirmDialogSubmit() {
      ...
      new Promise((resolve, reject) => { 
        ...
        // Send notification
        EventBus.$emit('send-notification', 'Server variable changed', '#00b16a')
