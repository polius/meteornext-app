  ...
  <v-row no-gutters>
    <v-col class="flex-grow-1 flex-shrink-1">
      <v-tabs v-model="currentConn" v-if="Object.keys(server).length != 0 || connections.length > 1" show-arrows dense background-color="#2c2c2c" color="white" slider-color="#969696" slider-size="1" slot="extension" class="elevation-2" style="border-bottom: 1px solid #424242;">
        <draggable v-model="connections" class="v-tabs__container" @start="dragConnectionStart" @end="dragConnectionEnd">
          <v-tab v-for="(conn, index) in connections" :key="index" @click="changeConnection(index)" :title="Object.keys(conn.server).length > 0 ? '[' + conn.server.type + ' ' + conn.server.version + '] ' + conn.server.host : ''" style="padding:0px 10px 0px 0px; float:left; height:100%; text-transform:none;">
    ...
    <v-col cols="auto" class="flex-grow-0 flex-shrink-0">
      <div v-if="sidebarMode == 'objects' && headerTabSelected == 'client'" style="background-color:#2c2c2c; padding: 6px 0px 6px 6px; border-bottom: 1px solid #424242;">
        <v-btn :loading="clientQueryExecuting" :disabled="clientQuery.length == 0" @click="explainQuery()" title="Explain Query"><v-icon small>fas fa-chart-pie</v-icon></v-btn>
      </div>
    </v-col>
    <v-col cols="auto" class="flex-grow-0 flex-shrink-0">
