   ...
   onGridReady(params) {
    ...
    exportRowsSubmit() {
      ...
      if (this.exportFormat == 'Meteor') {
        let exportData = 'var DATA = ' + JSON.stringify(this.objectsItems.procedures) + ';\n' + 'var COLUMNS = ' + JSON.stringify(this.objectsHeaders.procedures.map(x => x.headerName.trim())) + ';'
        ...
        var SqlString = require('sqlstring');
        let rawQuery = 'INSERT INTO `<table>` (' + this.objectsHeaders.procedures.map(x => '`' + x.headerName.trim() + '`').join() + ')\nVALUES\n'
        let values = ''
        let args = []
