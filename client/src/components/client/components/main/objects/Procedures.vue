<template>
  <div style="height:100%">
    ...
    <!-- PROCEDURES -->
    <!---------------->
    <ag-grid-vue ref="agGridObjectsProcedures" suppressDragLeaveHidesColumns suppressFieldDotNotation suppressContextMenu preventDefaultOnContextMenu suppressColumnVirtualisation oncontextmenu="return false" @grid-ready="onGridReady" @new-columns-loaded="onNewColumnsLoaded" @cell-key-down="onCellKeyDown" @cell-context-menu="onContextMenu" style="width:100%; height:calc(100% - 84px);" class="ag-theme-alpine-dark" rowHeight="35" headerHeight="35" rowSelection="multiple" rowDeselection="true" :stopEditingWhenCellsLoseFocus="true" :columnDefs="objectsHeaders.procedures" :rowData="objectsItems.procedures"></ag-grid-vue>
...
export default {
  ...
  methods: {
   onGridReady(params) {
    ...
    resizeTable() {
      var allColumnIds = [];
      this.columnApi.objects.procedures.getColumns().forEach(function(column) {
    ...
    onCellKeyDown(e) {
      if (e.event.key == "c" && (e.event.ctrlKey || e.event.metaKey)) {
        ...
        let cell = this.gridApi.objects.procedures.getFocusedCell()
        let row = this.gridApi.objects.procedures.getDisplayedRowAtIndex(cell.rowIndex)
    ...
    onContextMenu(e) {
      if (!this.gridApi.objects.procedures.getSelectedNodes().some(x => x.id == e.node.id)) this.gridApi.objects.procedures.deselectAll()
