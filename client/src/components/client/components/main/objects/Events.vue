<template>
  <div style="height:100%">
    ...
    <!-- EVENTS -->
    <!------------>
    <ag-grid-vue ref="agGridObjectsEvents" suppressDragLeaveHidesColumns suppressFieldDotNotation suppressContextMenu preventDefaultOnContextMenu suppressColumnVirtualisation oncontextmenu="return false" @grid-ready="onGridReady" @new-columns-loaded="onNewColumnsLoaded" @cell-key-down="onCellKeyDown" @cell-context-menu="onContextMenu" style="width:100%; height:calc(100% - 84px);" class="ag-theme-alpine-dark" rowHeight="35" headerHeight="35" rowSelection="multiple" rowDeselection="true" :stopEditingWhenCellsLoseFocus="true" :columnDefs="objectsHeaders.events" :rowData="objectsItems.events"></ag-grid-vue>
    ...
    <div style="height:35px; background-color:#303030; border-top:2px solid #2c2c2c;">
      <v-row no-gutters style="flex-wrap: nowrap;">
        ...
        <v-col cols="auto">
          <v-btn :disabled="objectsItems.events.length == 0" @click="exportRows" text small title="Export rows" style="height:30px; min-width:36px; margin-top:1px; margin-left:2px; margin-right:2px;"><v-icon small style="font-size:13px;">fas fa-arrow-down</v-icon></v-btn>
          <span style="background-color:#424242; padding-left:1px;margin-left:1px; margin-right:1px;"></span>
        </v-col>
...
export default {
  ...
  methods: {
   onGridReady(params) {
    ...
    resizeTable() {
      var allColumnIds = [];
      this.columnApi.objects.events.getColumns().forEach(function(column) {
    ...
    onCellKeyDown(e) {
      if (e.event.key == "c" && (e.event.ctrlKey || e.event.metaKey)) {
        let selectedRows = this.gridApi.objects.events.getSelectedRows()
        if (selectedRows.length > 1) {
          ...
          // Apply effect
          // this.gridApi.objects.events.flashCells({
          //   rowNodes: this.gridApi.objects.events.getSelectedNodes(),
          //   flashDelay: 200,
          //   fadeDelay: 200,
          // })
        }
          ...
          this.gridApi.objects.events.flashCells({
            rowNodes: this.gridApi.objects.events.getSelectedNodes(),
            columns: [this.gridApi.objects.events.getFocusedCell().column.colId],
            flashDelay: 200,
            fadeDelay: 200,
          })
        }
    ...
    onContextMenu(e) {
      if (!this.gridApi.objects.events.getSelectedNodes().some(x => x.id == e.node.id)) this.gridApi.objects.events.deselectAll()
    ...
    contextMenuClicked(item) {
      ...
      else if (item == 'Select All') this.gridApi.objects.events.selectAll()
      else if (item == 'Deselect All') this.gridApi.objects.events.deselectAll()
    },
    copySQL() {
    ...
    exportRowsSubmit() {
      ...
      if (this.exportFormat == 'Meteor') {
        let exportData = 'var DATA = ' + JSON.stringify(this.objectsItems.events) + ';\n' + 'var COLUMNS = ' + JSON.stringify(this.objectsHeaders.events.map(x => x.colId)) + ';'
        this.download('export.js', exportData)
      }
      else if (this.exportFormat == 'JSON') {
        let exportData = JSON.stringify(this.objectsItems.events)
        this.download('export.json', exportData)
      }
      else if (this.exportFormat == 'CSV') {
        let replacer = (key, value) => value === null ? undefined : value
        let header = Object.keys(this.objectsItems.events[0])
        let exportData = this.objectsItems.events.map(row => header.map(fieldName => JSON.stringify(row[fieldName], replacer)).join(','))
        ...
        var SqlString = require('sqlstring');
        let rawQuery = 'INSERT INTO `<table>` (' + this.objectsHeaders.events.map(x => '`' + x.colId + '`').join() + ')\nVALUES\n'
        let values = ''
        let args = []
        for (let row of this.objectsItems.events) {
          let rowVal = Object.values(row)
          args = [...args, ...rowVal];
          values += '(' + '?,'.repeat(rowVal.length).slice(0, -1) + '),\n'
        }
