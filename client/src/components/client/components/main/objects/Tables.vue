   ...
   onGridReady(params) {
    ...
    exportRowsSubmit() {
      ...
      if (this.exportFormat == 'Meteor') {
        let exportData = 'var DATA = ' + JSON.stringify(this.objectsItems.tables) + ';\n' + 'var COLUMNS = ' + JSON.stringify(this.objectsHeaders.tables.map(x => x.headerName.trim())) + ';'
        ...
        var SqlString = require('sqlstring');
        let rawQuery = 'INSERT INTO `<table>` (' + this.objectsHeaders.tables.map(x => '`' + x.headerName.trim() + '`').join() + ')\nVALUES\n'
        let values = ''
        let args = []
