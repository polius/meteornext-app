<template>
  <div style="height:100%">
    ...
    <!-- TABLES -->
    <!------------>
    <ag-grid-vue ref="agGridObjectsTables" suppressDragLeaveHidesColumns suppressFieldDotNotation suppressContextMenu preventDefaultOnContextMenu suppressColumnVirtualisation oncontextmenu="return false" @grid-ready="onGridReady" @new-columns-loaded="onNewColumnsLoaded" @cell-key-down="onCellKeyDown" @cell-context-menu="onContextMenu" style="width:100%; height:calc(100% - 84px);" class="ag-theme-alpine-dark" rowHeight="35" headerHeight="35" rowSelection="multiple" rowDeselection="true" :stopEditingWhenCellsLoseFocus="true" :columnDefs="objectsHeaders.tables" :rowData="objectsItems.tables"></ag-grid-vue>
    ...
    <div style="height:35px; background-color:#303030; border-top:2px solid #2c2c2c;">
      <v-row no-gutters style="flex-wrap: nowrap;">
        ...
        <v-col cols="auto">
          <v-btn :disabled="objectsItems.tables.length == 0" @click="exportRows" text small title="Export rows" style="height:30px; min-width:36px; margin-top:1px; margin-left:2px; margin-right:2px;"><v-icon small style="font-size:13px;">fas fa-arrow-down</v-icon></v-btn>
          <span style="background-color:#424242; padding-left:1px;margin-left:1px; margin-right:1px;"></span>
        </v-col>
...
export default {
  ...
  watch: {
    headerTabSelected(val) {
      if (val == 'objects') {
        this.$nextTick(() => {
          if (this.gridApi.objects.tables != null) this.resizeTable()
        })
      }
    },
  ...
  methods: {
   onGridReady(params) {
      ...
      this.columnApi.objects.tables = params.columnApi
      this.$refs['agGridObjectsTables'].$el.addEventListener('click', this.onGridClick)
      this.gridApi.objects.tables.showLoadingOverlay()
    },
    onNewColumnsLoaded() {
      if (this.gridApi.objects.tables != null) this.resizeTable()
    },
    onGridClick(event) {
      if (event.target.className == 'ag-center-cols-viewport') {
        this.gridApi.objects.tables.deselectAll()
      }
    },
    resizeTable() {
      var allColumnIds = [];
      this.columnApi.objects.tables.getAllColumns().forEach(function(column) {
        allColumnIds.push(column.colId);
      });
      this.columnApi.objects.tables.autoSizeColumns(allColumnIds);
    },
    onCellKeyDown(e) {
      if (e.event.key == "c" && (e.event.ctrlKey || e.event.metaKey)) {
        let selectedRows = this.gridApi.objects.tables.getSelectedRows()
        if (selectedRows.length > 1) {
          ...
          // Apply effect
          // this.gridApi.objects.tables.flashCells({
          //   rowNodes: this.gridApi.objects.tables.getSelectedNodes(),
          //   flashDelay: 200,
          //   fadeDelay: 200,
          // })
        }
          ...
          this.gridApi.objects.tables.flashCells({
            rowNodes: this.gridApi.objects.tables.getSelectedNodes(),
            columns: [this.gridApi.objects.tables.getFocusedCell().column.colId],
            flashDelay: 200,
            fadeDelay: 200,
          })
        }
        ...
        let cell = this.gridApi.objects.tables.getFocusedCell()
        let row = this.gridApi.objects.tables.getDisplayedRowAtIndex(cell.rowIndex)
        let node = this.gridApi.objects.tables.getRowNode(row.id)
        this.gridApi.objects.tables.deselectAll()
        node.setSelected(true)
      }
    },
    onContextMenu(e) {
      if (!this.gridApi.objects.tables.getSelectedNodes().some(x => x.id == e.node.id)) this.gridApi.objects.tables.deselectAll()
    ...
    contextMenuClicked(item) {
      ...
      else if (item == 'Select All') this.gridApi.objects.tables.selectAll()
      else if (item == 'Deselect All') this.gridApi.objects.tables.deselectAll()
    },
    copySQL() {
    ...
    refresh() {
      new Promise((resolve, reject) => {
        ...
        this.gridApi.objects.tables.hideOverlay() 
        this.loading = false 
      })
    },
    exportRows() {
    ...
    exportRowsSubmit() {
      ...
      if (this.exportFormat == 'Meteor') {
        ...
        let replacer = (key, value) => value === null ? undefined : value
        let header = Object.keys(this.objectsItems.tables[0])
        let exportData = this.objectsItems.tables.map(row => header.map(fieldName => JSON.stringify(row[fieldName], replacer)).join(','))
        ...
        var SqlString = require('sqlstring');
        let rawQuery = 'INSERT INTO `<table>` (' + this.objectsHeaders.tables.map(x => '`' + x.colId + '`').join() + ')\nVALUES\n'
        let values = ''
        let args = []
        for (let row of this.objectsItems.tables) {
          let rowVal = Object.values(row)
          args = [...args, ...rowVal];
          values += '(' + '?,'.repeat(rowVal.length).slice(0, -1) + '),\n'
        }
