  ...
  methods: {
    ...
    dialogSubmit() {
      ...
      if (this.dialogOptions.mode == 'new') {
        ...
        // Build query
        if (this.dialogOptions.item.type == 'PRIMARY') query = "ALTER TABLE `" + this.sidebarSelected[0]['name'].replaceAll('`','``') + "` ADD PRIMARY KEY (" + this.dialogOptions.item.fields + ");"
        else query = "ALTER TABLE `" + this.sidebarSelected[0]['name'].replaceAll('`','``') + "` ADD " + this.dialogOptions.item.type + ' `' + this.dialogOptions.item.name + "` (" + this.dialogOptions.item.fields + ");"
      }
      else if (this.dialogOptions.mode == 'delete') {
        let row = this.gridApi.structure.indexes.getSelectedRows()[0]
        if (row.Type == 'PRIMARY') query = "ALTER TABLE `" + this.sidebarSelected[0]['name'].replaceAll('`','``') + "` DROP PRIMARY KEY;"
        else query = "ALTER TABLE `" + this.sidebarSelected[0]['name'].replaceAll('`','``') + "` DROP INDEX `" + row.Name + '`;'
      }
      // Execute query
      this.execute(query)
    },
